...
def GetTrajectory(r_0,pr_0,xi_0,pz_0):
  ...
  r_dat, z_dat, t_dat, xi_dat, p_dat = [],np.array([]),np.array([]),[],np.array([])
  ...
  while True:
    if old_p < p and xin > xi_sim[-1]/2.0 and p < 0.1:
      break
...
def plotTest():
  ...
  for i in range(len(trajectories)):
    ...
    axs[1].plot(xi,p,'b',alpha=0.5,linewidth=1)
  axs[0].set_xlim(xi_sim[0], xi_sim[-1])
  axs[1].set_xlim(xi_sim[0], xi_sim[-1])
  axs[0].set_title('Backtracked Electron Trajectories')
  axs[1].axhline()
