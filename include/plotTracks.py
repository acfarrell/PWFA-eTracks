...
def plotBackTrack(r, xi, r_back,xi_back, E, r_sim, xi_sim, SHM, track):
  ...
  if SHM:
    ...
    ax.plot(xi,r,'k',label = "Simulated Forward")
  #plt.plot(xi[-1],r[-1],'ro', label = "Simulated ($\\xi_f,r_f$)")
  plt.xlim(xi_sim[0], xi_sim[-1])
  plt.ylim(r_sim[0], r_sim[-1])
  ax.plot(xi_back, r_back, 'c--', label="Simulated Backward")
  #plt.plot(xi_OSIRIS[-1],r_OSIRIS[-1],'bo', label = "OSIRIS ($\\xi_f,r_f$)")
  ax.legend()
  if SHM:
    model = "SHM"
