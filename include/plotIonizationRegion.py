...
def Wdt(E0):
  ...
  sigmaz = .94 # beam spread in z
  dt = 3 * sigmaz * WP / C # maximum duration of ionization
  
  E = E0 * (M_E * C * WP)/EC * 1e-9  #Convert normalized field to GV/m

  W0 = 1.52e15 * 4**n * gsE / (n * math.gamma(2*n))
  
  W = W0*(20.5*gsE**(3/2)/E)**(2*n-1)*math.exp(-6.83*gsE**(3/2)/E)
  print(E)
  return W * dt
...
def plotIonizationRegion():
  ...
  colors = ax.pcolormesh(xi ,r,E * M_E * C * WP / EC,norm=col.SymLogNorm(linthresh=0.03,linscale=0.03,vmin=-E.max()*M_E * C*WP/EC,vmax=E.max()*M_E*C*WP/EC),cmap="RdBu_r")
